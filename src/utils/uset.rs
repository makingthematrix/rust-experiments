...
/// the size of the new vector is `max - min`: the minimum value is at vec[0] (so,
/// `offset == min`) and `max - offset == capacity`. However, for performance
...
/// let uset = uset![2, 4, 6];
/// assert!(uset.len() == 3);
/// assert!(uset.contains(2));
/// assert!(uset.contains(4));
/// assert!(uset.contains(6));
/// ```
#[allow(unused_macros)]
macro_rules! uset {
